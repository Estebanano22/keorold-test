...
exports.smallest = async (req, res) => {
  ...
  if(Math.min(...arrayA) < -100000 || Math.max(...arrayA) > 100000) return res.status(401).send({error: 'Un elemento dentro del arrayA no se encuentra dentro del rango [−1,000,000 ... 1,000,000].'});
  
  // Validar que los elementos del arrayA sean números y tambien que sean enteros
  if(arrayA.filter(i => isNaN(i) || !Number.isInteger(i)).length > 0) return res.status(401).send({error: 'Uno ó varios elementos dentro del arrayA no es numerico ó no es entero.'});
  ...
  const existeArray = await Arrays.findOne({ where: { array: arrayA }});

  // Almacenar en DB Arrays Verificados
  if (existeArray) return res.status(200).send({result: numeroMenor(arrayA), array: req.body.array});
...
exports.stats = async (req, res) => {
  ...
  const countArrays = await Arrays.count({ where: { [Op.and]:[{result:numeroEntero}] }});
